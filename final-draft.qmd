...
-   Donor pool: 51 countries with...
    ...
    -   \(1\) in OECD or classified by the World Bank as upper middle–income (UMC) or high-income (HIC) countries at the time of treatment in 2001
...
-   \(2\) Using per capita emissions to avoid interpolation bias. Per capita data normalizes across country population size so we do not compare emissions on different magnitudes across donor pool countries.
...
We used the {tidysynth} package to generate our synthetic control UK, as compared to the authors' optimized algorithm method which involved a manual econometrics approach. To follow the author's approach, we excluded 1980-1990 from our analysis as they found an inconsistent pre-treatment fit during this pre-treatment period. Without doing this, synthetic controls are generated using weights from a poor pre-treatment fit window. This  decreases overall pre-treatment fit of the synthetic control to the actual UK, decreasing the accuracy of synthetic control trajectory.
...
# Create a function to apply synthetic control over data sets
...
# Plots to visualize difference in synthetic and real UK 
# data_OECD
...
p3_data_OECD <- scm_data_OECD %>%
  plot_placebos(prune = FALSE) + # Zoom out to see all lines
...
# Kable tables of top donor weights from each synthetic control version
...
# Plotting top weights to compare vizually with author's findings
...
```
The authors of the original study report their top donor countries as 19% Poland, 19% Libya, 18% Bahamas, 16% Belgium, 6% Trinidad and Tobago, 5% Uruguay, 4% Luxembourg, and 1% Brunei. They used weighted averages of the donor pool countries and an algorithm to best match the pre-treatment UK and synthetic UK trends. Additionally, based on our gap comparison, our best fit donor pool is the widest OECD, HIC, and UMC pool. Comparing the output from this donor pool's synthetic control, our results are very similar with all the same countries contained, and Libya and Poland representing our top 2 donor countries. This means that our SCM donor pool will all the criteria (OECD, HIC, and UMC countries) and the study's donor pool are closely equivalent and the synthetic UK in both cases act similar which can be observed in the table and plot below.
...
# Create kable table to summarize MSPE for each synth version
...
### Replicating Pre-Treatment Differences Plot

![Lepissier and Mildenberger's weighed pre-treatment differences plot.](images/figure_4.png)
...
We created a fairly near replication of the Lépissier and Mildenberger (2021) synthetic control analysis of the UK's 2001 Climate Change Programme using the {tidysynth package} in R.

Our first approach averaged CO2 emissions per capita across the pre-treatment window by country, which produced a poor pre-treatment fit and thus was unsuitable as a counterfactual. Switching to yearly emissions by donor country substantially improved fit, particularly for the widest donor pool (OECD, HIC, and UMC countries), which we identified as our strongest specification.

Across all three donor pools, gap plots showed a consistently negative and widening post-treatment divergence, suggesting the CCP reduced UK emissions relative to the synthetic counterfactual. This compared closely to the results of the Lépissier and Mildenberger study, finding a 9.8% decrease in per capita CO2 emissions in 2005. 

However, the placebo tests told a more cautious story. While pre-treatment fit was strong for the UK, no clear post-treatment divergence emerged relative to the placebo distribution, meaning we cannot confidently attribute the gap to the policy intervention alone.

The MSPE ratios were large, with the UK ranking first across all donor pools. These should be interpreted carefully: our near-perfect pre-treatment fit means even modest post-treatment deviations inflate the ratio. Balance tables confirmed close alignment between the real and synthetic UK across all pre-treatment years, validating our synthetic control construction.

Taken together, our results support the synthetic control as a well-constructed counterfactual, but the robustness checks present a mixed picture on whether a causal treatment effect can be cleanly identified.
